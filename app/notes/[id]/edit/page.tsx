...
export default function EditNotePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchNote = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.type === 'general') setContent(data.content || '');
          if (data.type === 'dsa' && data.dsa) setDsa(data.dsa);
          if (data.type === 'qa' && data.qa) setQa(data.qa);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'notes', unwrappedParams.id), noteData);

      router.push(`/notes/${unwrappedParams.id}`);
    } catch (error) {
      handleFirestoreError(error, OperationType.UPDATE, `notes/${unwrappedParams.id}`);
              ...
              <header className="mb-10 flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-neutral-500 capitalize">{type} Note</p>
                  </div>
                </div>
